...
export type AssembleResultSuccess = { success: true } & Program & { hasORG: boolean; hasORG20hAtStart: boolean };
...
export function assemble(source: string): AssembleResult {
  try {
    ...
    return { success: true, data, instructions, hasORG: store.hasOriginDirective(), hasORG20hAtStart: store.hasORG20hAtStart() };
  } catch (error) {
