...
export function generateDataPath(
  ...
  if (options?.separateMBRPaths && from === "MBR" && (to === "left" || to === "right")) {
    ...
    console.log(
      `🎯 Usando ruta especial ${normalizedFrom} → MAR para OUT (evitando nodos de entrada)`,
    );
    path = [
      normalizedFrom,
      `${normalizedFrom} out`,
    ...
    // evitando completamente NodoRegIn y mbr reg join (solo pasa por NodoRegOut y outr mbr join)
    console.log(
      "🎯 Usando ruta prioritaria para OUT registro→MBR (evitando NodoRegIn y mbr reg join)",
    );
    path = [
      normalizedFrom,
      `${normalizedFrom} out`,
