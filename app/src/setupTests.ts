...
function ComputerContainerMock() {
  return React.createElement('div', { 'data-testid': 'computer-container' }, 'Computer Container Mock');
}
...
vi.mock("@/computer/cpu/atoms", () => ({
  ...
  spAtom: { init: { low: 0xFF, high: 0 }, read: vi.fn(() => ({ low: 0xFF, high: 0 })), write: vi.fn() },
  flagsAtom: { init: { Z: false, C: false, S: false }, read: vi.fn(() => ({ Z: false, C: false, S: false })), write: vi.fn() },
  // Ciclo de instrucción
  cycleAtom: { init: { metadata: {} }, read: vi.fn(() => ({ metadata: {} })), write: vi.fn() },
  // Memoria
  memoryAtom: { init: new Array(65536).fill(0), read: vi.fn(() => new Array(65536).fill(0)), write: vi.fn() },
  // Editor
...
expect.extend({
  toHaveBeenCalledWithMatch(received, ...expected) {
    ...
    return {
      pass,
      message: () => `expected ${received.getMockName()} to have been called with arguments matching ${JSON.stringify(expected)}`,
    };
  },
});
