...
export const StepByStepMode = memo(
  ({ steps, onStepChange, onComplete, className }: StepByStepModeProps) => {
    ...
    return (
      <div className={clsx("rounded-lg border border-stone-600 bg-stone-900 p-4", className)}>
        {/* Header */}
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-mantis-400">Modo Paso a Paso</h3>
        ...
        <ExecutionPhases currentPhase={currentStep?.phase} className="mb-4" />

        {/* Contenido del paso actual */}
        <div className="mb-4 rounded-lg bg-stone-800 p-4">
          <div className="mb-2 flex items-center gap-2">
            <span className="text-sm font-medium text-mantis-400">{currentStep?.title}</span>
          </div>
          ...
          {currentStep?.instruction && (
            <div className="mb-3 rounded bg-stone-700 p-2">
              ...
              <div className="font-mono text-sm text-mantis-400">{currentStep.instruction}</div>
            </div>
          )}
