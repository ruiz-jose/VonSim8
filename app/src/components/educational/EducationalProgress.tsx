...
export const EducationalProgress = memo(({ className, isVisible = false, onClose }: EducationalProgressProps) => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('educational-progress', handleProgressEvent as EventListener);
  }, [updateProgress]);
      ...
      <div className="h-64 space-y-3 overflow-y-auto p-3">
        ...
        <div>
          ...
          <div className="text-xs text-stone-400">
            <div>Tiempo de sesión: {Math.floor(sessionTime / 60)}m {sessionTime % 60}s</div>
            <div>Niveles completados: {unlockedLevels.length}/{levels.length}</div>
          </div>
        </div>
      </div>
    </div>
