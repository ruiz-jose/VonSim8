...
Cuando el [reloj](../devices/clock) hace tic, se incrementa el registro `CONT` en uno. Empieza a contar luego de la primera escrita al registro `CONT`. Cuando el registro `CONT` coincide con el registro `COMP`, se dispara una interrupción por la línea `IRQ1` del [PIC](./pic).
